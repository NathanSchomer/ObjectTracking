...
int main(int argc, char *argv[])
{
	...
	ros::NodeHandle n;
	ros::Publisher x_pub = n.advertise<std_msgs::Float32>("x_pos", 1);
	ros::Publisher y_pub = n.advertise<std_msgs::Float32>("y_pos", 1);
	ros::Rate loop_rate(50); //50 hertz
        ...
        if(input_video.retrieve(cap_img))
        {
	    	...
	    	if(!got_roi) waitKey(0);
	    		...
	    		if(frame_num == 0){
					...
					MatchingMethod( cap_img, roid2, &x, &y);
					x_pub.publish(x);
					y_pub.publish(y);
				}
				waitKey(1);
...
void MatchingMethod( Mat img, Mat templ, int &x, int &y)
{
